...
<hr>
This repository utilizes a GitHub action partially based on the one <a href="https://github.com/m-nolan/doge-scrape"> published by m-nolan</a>, but with some slight modifications. Biggest difference: This action uses an R-based scraper that pulls from the DOGE API, and then utilizes the Python script by m-nolan to parse the FPDS information for the contracts data. The goal is to run the scraper weekly* to capture new changes, but currently I am running it manually because this set up requires checking the number of 500 result pages the API will return and editing the script to scrape each one. <br>
